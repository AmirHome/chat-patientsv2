...
class Setting extends Model
{
    ...
    public function getLogoUrl($value, string $path = self::PATH): string
    {
        if (! empty($value)) {
    ...
    public function getNotificationSound($value, string $path = self::NOTIFICATION_SOUND_PATH): string
    {
        if (! empty($value)) {
            return $this->imageUrl($path.DIRECTORY_SEPARATOR.$value);
        }
